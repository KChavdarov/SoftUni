...
const endpoints = {
    ...
    COMMENTS_BY_ID: 'data/comments?where=',
...
export async function getCommentsById(id){
    return await api.get(endpoints.COMMENTS_BY_ID + encodeURIComponent(`recipeId="${id}"`)+ '&load=' + encodeURIComponent('author=_ownerId:users'));
}
